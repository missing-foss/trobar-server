...
"""#189: playlist mirroring's third sink — a Jellyfin server, alongside
app/mirror.py's filesystem sink and app/mirror_subsonic.py's Subsonic sink.
Same contract, same "one-way, golden-wins, full idempotent rewrite"
semantics, for any playlist with `jellyfin_mirror_enabled = 1`.
...
(`jellyfin_mirror_remote_id`) — there's no ambient shared namespace a stray
...
dance a "replace" decomposes into since Jellyfin has no one-shot
create-or-replace call, best-effort name+comment) live in jellyfin_client.py's
mirror_*() functions, confirmed live against a real Jellyfin 10.11.11
instance."""
...
# Jellyfin's own HTTP status for "that playlistId doesn't exist" (confirmed
# live: every playlist-scoped call returns a clean 404, body "Playlist not
# found", for a stale/nonexistent id) — a universal HTTP code, unlike
# Subsonic's protocol-specific numeric error.code, so no named export from
# jellyfin_client is needed for this.
_ERROR_NOT_FOUND = 404


def _set_error(conn, playlist_id: int, code: str, detail: str | None = None) -> None:
    conn.execute(
        "UPDATE playlists SET jellyfin_mirror_last_error = ?, "
        "jellyfin_mirror_last_error_code = ? WHERE id = ?",
...
def _get_tag_index(tag_index_cache: dict | None):
    ...
    if tag_index_cache is None:
        return jellyfin_client.mirror_build_tag_index()
    if "index" not in tag_index_cache:
        tag_index_cache["index"] = jellyfin_client.mirror_build_tag_index()
...
def delete_mirror(conn, playlist_id: int) -> None:
    ...
    jellyfin_mirror_enabled flips to 0, and from playlist_sync.py's
    stale-playlist cleanup (BEFORE the playlist row itself is deleted,
    since this needs jellyfin_mirror_remote_id first). Does not commit —
    ...
    row = conn.execute(
        "SELECT jellyfin_mirror_remote_id FROM playlists WHERE id = ?", (playlist_id,)
    ).fetchone()
    if row is None or not row["jellyfin_mirror_remote_id"]:
        return
    if not jellyfin_client.mirror_delete_playlist(row["jellyfin_mirror_remote_id"]):
        _log.warning(
            "failed to delete remote Jellyfin mirror playlist %s for playlist %s",
...
def write_mirror(conn, playlist_id: int, tag_index_cache: dict | None = None) -> None:
    """Idempotent full rewrite of this playlist's Jellyfin mirror. No-ops
    (touches nothing) if the playlist isn't jellyfin_mirror_enabled or no
    mirror-target connection is configured. Never raises — every failure
    mode is instead persisted to jellyfin_mirror_last_error for the admin
    overview, same contract as mirror.write_mirror()/mirror_subsonic.write_
    mirror(). Does not commit — runs inline in playlist_sync.py's
    per-playlist commit.
    ...
    # to happen up front here instead. Same split mirror.py/mirror_subsonic.py
    # already make (unset_folder/unset_target vs a real failure).
    if db.get_mirror_jellyfin_config() is None:
    ...
    # same as the other two sinks would.
    ...
    jellyfin_client.mirror_set_playlist_metadata(
        remote_id, row["title"],
