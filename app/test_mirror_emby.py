...
"""Tests for mirror_emby.py's playlist-mirroring fourth and (per the RFC)
final sink (#189): enabled-gating, the tag-based target lookup, the
collapsed-match guard, the stale-remote-id recreate path, the write/delete
DB bookkeeping, and the admin-error-surfacing failure modes. emby_client's
own request mechanics (pagination, replace semantics, the three confirmed
divergences from Jellyfin's own mirror_*() functions, ...) are covered
directly in test_emby_client.py — mocked at that module boundary here,
same as test_mirror_jellyfin.py mocks jellyfin_client one layer up. Same
DATA_DIR/DB_PATH tmp-dir override pattern as test_mirror_subsonic.py.

    python3 -m unittest test_mirror_emby -v      # from app/
"""
import shutil
import tempfile
import unittest
...
class _MirrorEmbyTestBase(unittest.TestCase):
    def setUp(self):
        self._tmp = tempfile.mkdtemp(prefix="trobar-test-mirror-emby-")
        ...
        self._track_seq = 0
        # write_mirror() checks db.get_mirror_emby_config() directly
        # (not through the mocked emby_client) to distinguish "unset"
        # from "configured but unreachable" — every test needs a real
        # config on record unless it's specifically testing the unset case.
        db.set_config(self.conn, "mirror_emby_url", "http://mirror.example.com")
        db.set_config(self.conn, "mirror_emby_api_key", "key")
        db.set_config(self.conn, "mirror_emby_username", "trobar")
    ...
    def _clear_mirror_target_config(self) -> None:
        db.set_config(self.conn, "mirror_emby_url", None)
        db.set_config(self.conn, "mirror_emby_api_key", None)
        db.set_config(self.conn, "mirror_emby_username", None)
        db.set_config(self.conn, "mirror_emby_user_id", None)
        self.conn.commit()

    def _make_playlist(self, title: str, emby_mirror_enabled: bool = True,
                        remote_id: str | None = None) -> int:
        cur = self.conn.execute(
            "INSERT INTO playlists (title, emby_mirror_enabled, emby_mirror_remote_id) "
            "VALUES (?, ?, ?)",
            (title, 1 if emby_mirror_enabled else 0, remote_id),
        )
        self.conn.commit()
    ...
    def _row(self, playlist_id: int):
        return self.conn.execute(
            "SELECT emby_mirror_enabled, emby_mirror_remote_id, "
            "emby_mirror_last_written_at, emby_mirror_last_error, "
            "emby_mirror_last_error_code FROM playlists WHERE id = ?",
            (playlist_id,),
        ).fetchone()


class NoOpTests(_MirrorEmbyTestBase):
    def test_disabled_playlist_is_a_no_op(self):
        pid = self._make_playlist("Chill", emby_mirror_enabled=False)
        with mock.patch("mirror_emby.emby_client") as client:
            mirror_emby.write_mirror(self.conn, pid)
        client.mirror_build_tag_index.assert_not_called()
        self.assertIsNone(self._row(pid)["emby_mirror_remote_id"])
    ...
    def test_no_mirror_target_configured_sets_unset_target_and_never_calls_the_client(self):
        ...
        # db.get_mirror_emby_config(), before ever reaching
        # emby_client, so a missing config can't even look like a
        # network failure.
        ...
        with mock.patch("mirror_emby.emby_client") as client:
            ...
            client.mirror_build_tag_index.assert_not_called()
        row = self._row(pid)
        self.assertIsNone(row["emby_mirror_remote_id"])
        self.assertEqual(row["emby_mirror_last_error_code"], "unset_target")
    ...
    def test_configured_but_unreachable_target_sets_an_error(self):
        ...
        row = self._row(pid)
        self.assertIsNone(row["emby_mirror_remote_id"])
        self.assertEqual(row["emby_mirror_last_error_code"], "unreachable")


class WriteContentTests(_MirrorEmbyTestBase):
    def test_successful_write_stores_the_new_remote_id_and_clears_errors(self):
        ...
        row = self._row(pid)
        self.assertEqual(row["emby_mirror_remote_id"], "99")
        self.assertIsNotNone(row["emby_mirror_last_written_at"])
        self.assertIsNone(row["emby_mirror_last_error"])
        self.assertIsNone(row["emby_mirror_last_error_code"])
    ...
    def test_write_failure_sets_an_error_and_leaves_remote_id_untouched(self):
        ...
        with mock.patch("mirror_emby.emby_client") as client:
            ...
            mirror_emby.write_mirror(self.conn, pid)
        row = self._row(pid)
        self.assertEqual(row["emby_mirror_remote_id"], "1")  # unchanged
        self.assertEqual(row["emby_mirror_last_error_code"], "write_failed")
        self.assertEqual(row["emby_mirror_last_error"], "boom")


class CollapsedMatchGuardTests(_MirrorEmbyTestBase):
    ...
    def test_zero_target_matches_with_locally_matched_tracks_sets_an_error(self):
        ...
        row = self._row(pid)
        self.assertEqual(row["emby_mirror_last_error_code"], "no_target_matches")
        self.assertIsNone(row["emby_mirror_remote_id"])
    ...
    def test_a_playlist_with_nothing_matched_locally_still_writes_an_empty_target_playlist(self):
        ...
        with mock.patch("mirror_emby.emby_client") as client:
            ...
            client.mirror_create_or_replace_playlist.assert_called_once_with(
                "Empty So Far", [], None)
        self.assertIsNone(self._row(pid)["emby_mirror_last_error_code"])


class TrackNoTiebreakTests(_MirrorEmbyTestBase):
...
class StaleRemoteIdTests(_MirrorEmbyTestBase):
    """#189 review analog to the Subsonic/Jellyfin sinks: a remote id whose
    ...
    own delete-call failure. Emby's stale signal is a universal HTTP 404
    (mirror_emby._ERROR_NOT_FOUND) surfaced through emby_client's own
    up-front existence check (see that module's docstring for why a plain
    GET .../Items can't be trusted to 404 cleanly here) — from
    write_mirror()'s point of view this looks identical to Jellyfin's own
    404-on-replace signal, just produced a different way one layer down."""
    ...
    def test_not_found_clears_the_stale_id_and_recreates(self):
        ...
        row = self._row(pid)
        self.assertEqual(row["emby_mirror_remote_id"], "new-id")
        self.assertIsNone(row["emby_mirror_last_error_code"])
    ...
    def test_a_different_error_code_does_not_trigger_a_retry(self):
        ...
        row = self._row(pid)
        self.assertEqual(row["emby_mirror_remote_id"], "stale-id")  # untouched
        self.assertEqual(row["emby_mirror_last_error_code"], "write_failed")
    ...
    def test_a_missing_remote_id_never_retries_regardless_of_code(self):
        ...
        # place (write_mirror()'s own guard: `... and row["emby_
...
class TagIndexCacheTests(_MirrorEmbyTestBase):
    ...
    def test_a_shared_cache_builds_the_index_only_once(self):
        ...
        with mock.patch("mirror_emby.emby_client") as client:
            ...
            client.mirror_create_or_replace_playlist.return_value = {
                "status": "ok", "remote_id": "1"}
            mirror_emby.write_mirror(self.conn, pid1, tag_index_cache=cache)
            mirror_emby.write_mirror(self.conn, pid2, tag_index_cache=cache)
    ...
    def test_no_cache_builds_a_fresh_index_every_call(self):
        ...
        with mock.patch("mirror_emby.emby_client") as client:
            ...
            client.mirror_create_or_replace_playlist.return_value = {
                "status": "ok", "remote_id": "1"}
            mirror_emby.write_mirror(self.conn, pid1)
            mirror_emby.write_mirror(self.conn, pid2)
    ...
    def test_a_cached_failed_build_is_not_retried_within_the_same_run(self):
        ...
        with mock.patch("mirror_emby.emby_client") as client:
            ...
            mirror_emby.write_mirror(self.conn, pid2, tag_index_cache=cache)
        client.mirror_build_tag_index.assert_called_once()
        self.assertEqual(self._row(pid1)["emby_mirror_last_error_code"], "unreachable")
        self.assertEqual(self._row(pid2)["emby_mirror_last_error_code"], "unreachable")


class IdempotentRewriteTests(_MirrorEmbyTestBase):
...
class DeleteMirrorTests(_MirrorEmbyTestBase):
    ...
    def test_delete_removes_the_remote_playlist_and_clears_remote_id(self):
        ...
        with mock.patch("mirror_emby.emby_client") as client:
            ...
            mirror_emby.delete_mirror(self.conn, pid)
        client.mirror_delete_playlist.assert_called_once_with("42")
        row = self._row(pid)
        self.assertIsNone(row["emby_mirror_remote_id"])
        self.assertIsNone(row["emby_mirror_last_written_at"])
